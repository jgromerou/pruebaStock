...
exports.nuevafactura = (req, res) => {
  ...
  const items = body.facturas[0].items.map((item) =>
    Itemventa.findOrCreate({
      where: {
        id: item.id,
      },
      defaults: {
  ...
  Cliente.findByPk(body.id)
    ...
    .then((factura) =>
      Promise.all(items)
        .then((storedItems) => factura.addItems(storedItems))
        .then(() => factura)
    )
    .then((factura) =>
      Facturaventa.findOne({
        where: {
          id: factura.id,
        },
      })
    ...
    .then(
      (facturacompleta) => res.status(201).json(facturacompleta),
...
//busca una factura de venta por Id.
